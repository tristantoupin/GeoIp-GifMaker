...
def generate_map(output, lats=[], lons=[], wesn=None):
    ...
    for k in range(1, len(x)):
        m.scatter(x[:k], y[:k], s=1, c="white", marker='.', alpha=0.5)
        plt.plot_date(x[:k-1],y[:k-1],'b-',linewidth=1,c='grey')
        if (k < len(x)):
            plt.plot_date(x[k-1:k+1],y[k-1:k+1],'b-',linewidth=0.8,c="white")

        file_name = directory + "/image_" + chr(k + 96)
